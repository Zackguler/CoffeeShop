...
final class OrderSummaryViewController: UIViewController {
    ...
    private func setupUI() {
        ...
        titleLabel.text = "order_success_title".localized
        ...
        let dateText = formatter.string(from: viewModel.orderDate)

        orderInfoLabel.text = String(format: "order_info_format".localized, viewModel.orderId, dateText)
        orderInfoLabel.textColor = Colors().colorDarkGray
        orderInfoLabel.textAlignment = .center
        orderInfoLabel.font = UIFont.fontRegular16

        let totalPriceText = String(format: "%.2f", viewModel.totalPrice)
        totalLabel.text = String(format: "order_total_format".localized, totalPriceText)
        ...
        homeButton.setTitle("order_back_home_button".localized, for: .normal)
